...
io.on("connection", (socket) => {
  ...
  socket.on("user_online", async (username) => {
    ...
    try {
      const users = await User.find({}, "username lastSeen");   // 👈 DB se last seen, restart-proof
      const map = {};
  ...
  socket.on("leave_conversation", (convId) => socket.leave(convId));
    ...
    conv.lastMessage     = type === "image" ? "📷 Photo" : type === "file" ? "📄 File" : type === "voice" ? "🎤 Voice message" : content;   // 👈 voice label
